import type { Translations } from "./index";

export const en: Translations = {
  nav: {
    home: "Home",
    search: "Search",
    history: "History",
    uploads: "Uploads",
    stats: "Stats",
    settings: "Settings",
    help: "Help",
    queue: "Queue",
  },

  home: {
    nowPlaying: "Now Playing",
    nothingPlaying: "Nothing is playing",
    nothingPlayingDescription: "Search or pick a track from the history below",
    recentlyPlayed: "Recently Played",
    noHistory: "No history yet",
  },

  search: {
    placeholder: "Search songs or paste URL, then Enter",
    searchForMusic: "Search Music",
    searchPrompt: "Search for songs, artists, or URLs",
    noResults: "No results",
    noResultsHint: "Try a different search term or source",
    youtube: "YouTube",
    historySource: "History",
    resultsCount: "{n} results",
    loadMore: "Load more",
    loadingMoreResults: "Loading more results",
    allResultsLoaded: "All {n} results loaded",
  },

  queue: {
    title: "Queue",
    nowPlaying: "Now Playing",
    upNext: "Up Next",
    empty: "Queue is empty",
    downloading: "Downloading",
    searchForMusic: "Search Music",
    playNow: "Play {title} now",
    removeFromQueue: "Remove {title} from queue",
  },

  player: {
    play: "Play",
    pause: "Pause",
    skip: "Skip",
    previous: "Previous",
    mute: "Mute",
    unmute: "Unmute",
    loop: "Loop: {mode}",
    seek: "Seek",
    volume: "Volume",
    toggleQueue: "Toggle queue",
    enableVolumeBoost: "Enable volume boost",
    disableVolumeBoost: "Disable volume boost",
    volumeBoostOn: "Volume boost on (0\u2013100%)",
    volumeBoostOff: "Volume boost off (0\u201310%)",
    defaultVolume: "Default volume",
    outputSuspended: "Waiting for voice output recovery",
  },

  settings: {
    title: "Settings",
    account: "Account",
    discordConnected: "Connected to Discord",
    logOut: "Log out",
    logoutFailed: "Global logout failed. Please try again.",
    appearance: "Appearance",
    theme: "Theme",
    dark: "Dark",
    light: "Light",
    system: "System",
    botControl: "Discord Bot",
    discordBot: "Discord Bot",
    restartDiscordBot: "Restart Discord Bot",
    requestingBotRestart: "Requesting restart",
    botStatusStarting: "Starting bot",
    botStatusRestarting: "Replacing bot",
    botStatusConnecting: "Connecting to the default voice channel",
    botStatusRetryWait: "Waiting to retry the voice connection",
    botStatusReady: "Ready",
    botStatusUnconfigured: "No default voice channel is configured",
    botStatusFailed: "Bot recovery failed",
    botStatusStopped: "Bot stopped",
    botStatusUnavailable: "Bot status is unavailable",
    botRecoveryTarget: "Recovery channel: {channel}",
    botRestartInProgress: "Bot replacement is in progress",
    restartAvailableIn: "Restart available in {seconds}s",
    voiceRetryIn: "Voice retry in {seconds}s",
    botRecoveryError: "Bot or voice connection error",
    persistenceError: "Playback recovery state is not being saved",
    botRestartFailed: "Could not request a bot restart",
    server: "Server",
    defaultVolume: "Default Volume",
    defaultVolumeDescription: "Applied to new tracks without a saved volume",
    volume: "Volume",
    saving: "Saving",
    saved: "Saved",
    defaultVoiceChannel: "Default Voice Channel",
    defaultVoiceChannelDescription:
      "The bot will auto-join this channel when playing from the web",
    noneManualJoinOnly: "None \u2014 manual join only",
    available: "{n} available",
    noVoiceChannels:
      "No voice channels available \u2014 the bot may not be connected yet",
    ytdlp: "yt-dlp",
    currentVersion: "Current version",
    notInstalled: "Not installed",
    usingSystemYtdlp: "Using system yt-dlp from PATH",
    checkForUpdates: "Check for updates",
    latestVersion: "Latest version",
    updateTo: "Update to {version}?",
    upToDate: "Up to date",
    youtubeApi: "YouTube API",
    apiKey: "API Key",
    notSet: "Not set",
    enterNewApiKey: "Enter new API key",
    save: "Save",
    apiKeySaved: "API key saved",
    historyChannel: "History Channel",
    historyChannelDescription:
      "Track history embeds will be posted to this channel",
    noneDisabled: "None \u2014 disabled",
    noTextChannels:
      "No text channels available \u2014 the bot may not be connected yet",
    timezone: "Timezone",
    timezoneDescription: "Used for stats heatmap and trend date grouping",
    comingSoon: "Coming soon",
    help: "Help",
    language: "Language",
    botLanguage: "Bot Language",
    botLanguageDescription: "Language used for Discord bot responses",
  },

  help: {
    title: "Help",
    discordCommands: "Discord Commands",
    webFeatures: "Web Features",
    keyboardShortcuts: "Keyboard Shortcuts",
    commandClusters: {
      playback: "Playback",
      settings: "Settings",
    },
    commands: {
      play: "Plays immediately if idle, adds to queue if playing",
      pause: "Pauses playback",
      resume: "Resumes playback",
      skip: "Skips to the next track",
      now: "Shows the currently playing track",
      volume: "Adjusts the volume",
      loop: "Sets the loop mode",
    },
    features: {
      home: "Recent play history and quick access",
      search: "Find tracks from YouTube or play history",
      player: "Control music playback",
      volumePerSong: "Volume is saved individually for each song",
      history: "View play history and re-add to queue",
      uploads: "View uploaded files and edit metadata",
      dragAndDrop: "Upload audio files via drag and drop",
      pasteToPlay: "Paste a link to add to queue",
      stats: "Heatmap, trends, top tracks & artists",
      settings: "Theme, language, and more",
    },
    featureLabels: {
      home: "Home",
      search: "Search",
      player: "Player",
      volumePerSong: "Per-song Volume",
      history: "History",
      uploads: "Uploads",
      dragAndDrop: "Drag & Drop",
      pasteToPlay: "Paste to Play",
      stats: "Stats",
      settings: "Settings",
    },
    shortcuts: {
      playPause: "Play / Pause",
      seekBackForward: "Seek back / forward 5s",
      volumeUpDown: "Volume up / down",
      muteUnmute: "Mute / Unmute",
      goToSearch: "Go to search",
    },
  },

  login: {
    continueWithDiscord: "Continue with Discord",
    serverMembersOnly: "Only server members can log in",
    notMemberError:
      "Only server members can log in. Please join the server first.",
  },

  auth: {
    checkFailedTitle: "Could not verify your login",
    checkFailedMessage:
      "The server could not be reached. Your login has been kept so you can try again.",
    forbiddenTitle: "Server access was denied",
    forbiddenMessage:
      "Check your server membership and try again. You have not been logged out automatically.",
    retry: "Check again",
  },

  history: {
    title: "History",
    empty: "No play history",
    emptyAction: "Start listening!",
    newRecordsBadge: "{n} new",
    unknownArtist: "Unknown artist",
    add: "Add",
    adding: "Adding",
    playCount: "{n} plays",
    tracksCount: "{n} tracks",
    loadMore: "Load more",
    loadingMoreTracks: "Loading more tracks",
    allTracksLoaded: "All {n} tracks loaded",
  },

  uploads: {
    title: "Uploads",
    uploadedTracks: "{n} uploaded tracks",
    empty: "No uploaded files",
    emptyHint: "Drag and drop audio files to upload",
    unknownArtist: "Unknown artist",
    addToQueue: "Add to queue",
    deleteTrack: "Delete track",
    cancel: "Cancel",
    deleteConfirm: "Delete?",
    newFilesBadge: "{n} new",
    tracksCount: "{n} tracks",
    loadMore: "Load more",
    loadingMoreTracks: "Loading more tracks",
    allTracksLoaded: "All {n} tracks loaded",
  },

  stats: {
    title: "Stats",
    empty: "No listening history yet",
    emptyAction: "Start listening!",
    failedToLoad: "Failed to load stats",
    retry: "Retry",
    plays: "plays",
    listened: "listened",
    tracks: "tracks",
    streak: "streak",
    streakDays: "{n}d",
    peak: "peak",
    listeningActivity: "Listening Activity",
    last30Days: "Last 30 Days",
    activityByDay: "Activity by Day",
    topTracks: "Top Tracks",
    topArtists: "Top Artists",
    playsCount: "{n} plays",
    tracksCount: "{n} tracks",
    durationHM: "{h}h {m}m",
    durationM: "{m}m",
    less: "Less",
    more: "More",
    dow: {
      mon: "Mon",
      tue: "Tue",
      wed: "Wed",
      thu: "Thu",
      fri: "Fri",
      sat: "Sat",
      sun: "Sun",
    },
    dowShort: {
      mon: "Mon",
      wed: "Wed",
      fri: "Fri",
      sun: "Sun",
    },
    months: {
      jan: "Jan",
      feb: "Feb",
      mar: "Mar",
      apr: "Apr",
      may: "May",
      jun: "Jun",
      jul: "Jul",
      aug: "Aug",
      sep: "Sep",
      oct: "Oct",
      nov: "Nov",
      dec: "Dec",
    },
  },

  toast: {
    addedToQueue: "Added to queue",
    removedFromQueue: "Removed from queue",
    failedToAddToQueue: "Failed to add to queue",
    failedToRemoveFromQueue: "Failed to remove from queue",
    failedToPlayTrack: "Failed to play track",
    failedToSkipTrack: "Failed to skip track",
    deleted: "Deleted",
    deleteFailed: "Delete failed",
    updateFailed: "Update failed",
    failed: "Failed",
    addToQueue: "Add to queue",
    apiKeySaved: "API key saved",
    failedToSave: "Failed to save",
    failedToCheckForUpdates: "Failed to check for updates",
    duplicateInQueue: "Already in queue",
    urlAddedToQueue: "Added to queue",
  },

  status: {
    reconnecting: "Reconnecting",
    loading: "Loading",
  },

  error: {
    failedToLoadStats: "Failed to load stats",
    unsupportedFileType: "Unsupported file type: {type}",
    unsupportedFileTypeGeneric: "Unsupported file type",
  },

  upload: {
    dropToUpload: "Drop to upload",
    dropFileHere: "Drop file here",
    supportedFormats: "MP3, FLAC, OGG, WAV, AAC, M4A, MP4, WebM",
    orSelectFile: "Or select file",
  },

  uploadModal: {
    title: "Upload to Queue",
    titleLabel: "Title",
    artistLabel: "Artist",
    titlePlaceholder: "Track title",
    artistPlaceholder: "Artist name",
    addToQueue: "Add to Queue",
    uploading: "Uploading\u2026",
    adding: "Adding\u2026",
    close: "Close",
    fileAlreadyUploaded: "File already uploaded",
    uploadNotComplete: "Upload not complete",
    retryMessage: "Upload failed \u2014 please select the file manually",
    selectFile: "Select File",
  },

  welcome: {
    subtitle: "A subtitle is a subtitle",
    viewHelp: "View Help",
    getStarted: "Get Started",
  },

  time: {
    justNow: "just now",
    minutesAgo: "{n}m ago",
    hoursAgo: "{n}h ago",
    daysAgo: "{n}d ago",
  },

  video: {
    noVideoAvailable: "No video available",
    youtubePlayer: "YouTube video player",
  },

  setup: {
    title: "Azuki Setup",
    titleReconfigure: "Azuki Reconfigure",
    subtitle: "Configure your Discord music bot",
    subtitleReconfigure: "Update your existing configuration",
    sectionDiscord: "Discord Credentials",
    sectionSecurity: "Session Security",
    sectionYoutube: "YouTube API (Optional)",
    placeholderClientId: "Application ID from Discord Developer Portal",
    placeholderClientSecret: "OAuth2 client secret",
    placeholderToken: "Bot token from Discord Developer Portal",
    placeholderGuildId: "Server ID (right-click server → Copy ID)",
    placeholderJwtSecret: "Random secret for JWT signing",
    placeholderYoutubeKey:
      "YouTube Data API v3 key (can set later in Settings)",
    placeholderSetupToken: "Paste the token shown in your terminal",
    setupTokenLabel: "SETUP TOKEN (from terminal)",
    generate: "Generate",
    remove: "Remove",
    youtubeKeyWillBeRemoved: "YouTube API key will be removed",
    reconfigureHint: "Leave fields empty to keep existing values",
    submit: "Complete Setup",
    submitReconfigure: "Update Configuration",
    saving: "Saving...",
    starting: "Starting Azuki...",
    serverDidNotRestart: "Server did not restart — check your terminal",
    connectionError: "Connection error — check your terminal",
  },

  common: {
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    retry: "Retry",
    loadMore: "Load more",
    unknownArtist: "Unknown artist",
    unknown: "Unknown",
    listening: "{n} listening",
  },
} as const;
