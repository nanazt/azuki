...
interface DownloadState {
  ...
  startDownload: (id: string, query: string) => void;
...
export const useDownloadStore = create<DownloadState>((set, get) => ({
  ...
  startDownload: (id, query) =>
    set((state) => {
      const next = new Map(state.downloads);
      next.set(id, { download_id: id, query, stage: "resolving", percent: 0, speed_bps: null, status: "downloading" });
