...
interface DownloadState {
  ...
  resolveMetadata: (id: string, meta: { title: string; artist: string | null; thumbnail_url: string | null; duration_ms: number; source_url: string }) => void;
  updateProgress: (id: string, stage: DownloadStage, percent: number, speed_bps: number | null) => void;
...
export const useDownloadStore = create<DownloadState>((set, get) => ({
  ...
  startDownload: (id, query, userInfo?) =>
    set((state) => {
      ...
      next.set(id, { download_id: id, query, stage: "resolving", percent: 0, speed_bps: null, status: "downloading", user_info: userInfo });
  ...
  completeDownload: (id, track) =>
    set((state) => {
      const next = new Map(state.downloads);
      const entry = next.get(id);
      if (entry) next.set(id, { ...entry, status: "complete", percent: 100, track });
      return { downloads: next };
    }),
